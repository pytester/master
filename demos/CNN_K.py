...
def add_square_layer(inputs, in_size, out_size):  
    ...
    b=tf.Variable(tf.random_normal([1, in_size]))
    c=tf.Variable(tf.ones([1, in_size]))

    f = c - a * tf.square(inputs - b)

    return tf.matmul(tf.nn.relu(f), tf.ones([input_size, out_size]))
