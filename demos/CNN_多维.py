...
# size(b_wight) = [out_size, in_size]
# inputs = [i1, i2, ...]
# b_wight = [[w1, w1, ...]
#            [w2, w2, ...]]  
def add_square_layer(inputs, b_wight):
    #out_size = len(b_wight)
    in_size  = len(b_wight[0])
    b=tf.fill(b_wight)
    f = 1 - tf.square(inputs - b)

    return tf.matmul(tf.nn.relu(f), tf.ones([in_size, 1])) / in_size
...
if __name__ == '__main__':
    ...
    long_xs = tf.placeholder(tf.float32,[None, long_dis], name = 'long_xs')
    ...
    for sz in size:#[-sz[0]:]
        #l1_mtx.append(
        l1, w1, b1 = add_square_layer(xs, sz[0], sz[1], activation_function=None)#, activation_function=tf.nn.relu, tf.nn.sigmoid))#激励函数(activation_function)ReLU  )
        l1_cnt += sz[1]
