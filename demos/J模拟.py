    ...
    def KDJ(self, N=30,M1=20,M2=20):
        ...
        for i in range(datelen):  
            ...
            if i==0:  
                ...
                min_low = float(min(cclose[b:i+1]))
                max_high = float(max(cclose[b:i+1]))
...
if __name__ == '__main__':
    ...
    #y_ = []
    start_idx = 30
    x_data = np.zeros((len(data.c) - start_idx, start_idx + 2), dtype=np.float32)
    ...
    for idx in range(start_idx, len(data.c)):
        ...
        x_data[idx - start_idx][-2] = max(x_data[idx - start_idx][:-2])
        x_data[idx - start_idx][-1] = min(x_data[idx - start_idx][:-2])
    ...
    for sz in size:#[-sz[0]:]
        #l1_mtx.append(
        l1, w1, b1 = add_layer(xs, sz[0], sz[1], activation_function=tf.nn.sigmoid)#, activation_function=tf.nn.relu, tf.nn.sigmoid))#激励函数(activation_function)ReLU  )
        l1_cnt += sz[1]
    ...
    train_step=tf.train.GradientDescentOptimizer(0.001).minimize(loss)#梯度下降优化器,减少误差，学习效率0.1  
