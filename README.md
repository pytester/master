...
PyTester是一个股票策略回测的平台.<br>
适用于一般的股民朋友. <br>
使用策略来验证自已的交易想法, 可以缩短摸索市场的时间，少交学费，降低入市的成本. <br>

---
* 测试平台: www.pytester.com
* 浏览器要求: 建意使用最新的IE10以上或firefox, chrome浏览器打开, 达到紧好效果。
...
2. 数据分析．
3. 股票筛选．
4. 市场是随机和不可预测的. 分析历史并不能帮你预测未来，它能做的是，当行情来时，第一时间心知肚明。
...
* 注意: 这里并不是以开盘和收盘价计算的KDJ. 而是以差值为数据, 计算的KDJ.
...
>每一个策略有自已的参数．每次运行时，可以使用不同的参数.<br>
>在代码行尾用 #@ 标识.<br>
...
>以上示例中并没涉及买卖的动作.<br>
>当您需要买入或卖出. 可把这些操作放到onTick()函数里面.<br>
>每个周期，会调一次onTick().<br>

* 返回值:<br>
    1. onTick()的返回值比较特殊, 为一个数字. 
    2. 表示当前需要加仓, 或平仓的手数或百分比. 相当于本次变动的仓位.<br>
        如return x<br>
        有如下规则:<br>
...
>若您需要在策略完成之后, 做一些事情. 可以把这部分代码放在onLast()里面.<br>
>策略计算完后，会调用一次onLast().<br>
>格式如下:<br>
...
* 如:<br>
    见[策略选股](#策略选股)示例中, onLast()返回两个数字: g.profit()和len(k) – g[-1][3]
...
* show=(ma, ema), (k, 50)<br>
    第一窗口画ma和ema, 第二窗口画k和50线, 第三窗口是总资产(本示例中无变化).
![](/images/ema_k50.jpg)

* show=None, (k, 50)
    第一窗口只有默认的收盘价, 第二窗口画k和50线, 第三窗口是总资产(本示例中无变化).
![](/images/None_k50.jpg)
...
通过下标来取值.  
假设周期为天, ma[-1], ma[-2] 代表今天和昨天的均值.

* 下标:<br>
仅支持负数, 表示的意义为: 从今天开始, 往回倒数.
...
### 经典指标
#### KDJ
KDJ(N, M1, M2) 由K线,D线和J线,这几条指标线组成.<br>
...
* 查看交易记录<br>
    g本身是一个数组. 当每一次调用onTick()返回非空时, 会记录到g变量.

|g|数据|
|-------|----------------------|
|g[-1][0]|上一次交易onTick()返回值|
|g[-1][2]|上一次交易时的价格   |
...
若您不清楚一些函数的用法， 可使用help(xx)的形式来获取帮助。结果会显示在后台输出界面中。
...
* 引入其它周期.
    * week = new(W1)      -> 引入当前合约一周为周期.
    * day  = new(D1)      -> 引入当前合约一天为周期
