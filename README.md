...
PyTester是一个股票策略回测的平台. 一般的股民朋友只要稍加学习,就能上手.  

通过回测历史数据来验证自已的交易想法, 缩短成熟交易方法的打磨时间, 帮您快速找到适合自已的方法.

同时从技术分析的角度帮您理解市场, 缩短摸索市场的时间，少交学费，降低入市的成本.

市场是随机和不可预测的. 历史回测并不是万能, 它不能帮你预测未来，它能做的是，当行情来时，第一时间警觉. 

和提高获胜的概率.

至于能不能抓住机遇, 还要看股民朋友自已的心态和胆量, 能否严格按照策略操作.

---
* 测试平台: www.pytester.com
* 浏览器要求: 建意使用最新的IE10以上或firefox, chrome浏览器打开, 达到最好的效果。
...
1. 鼠标移致顶部会出现:<br>
    运行, 保存, 打开, 批量计算, 批量结果, 后台输出, 教程, 代码视图切换.
